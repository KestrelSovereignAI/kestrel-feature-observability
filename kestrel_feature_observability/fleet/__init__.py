...
module. The ``[fleet]`` extra tightens the SDK pin (``>=0.32.0,<0.33``) to the
...
host env, or any base env whose SDK resolved to ``>=0.32.0``) it binds the real
...
try:
    ...
    logger.warning(
        ...
        ">=0.32.0,<0.33, e.g. via the [fleet] extra).",
        exc,
    )
